...
def process_withdrawal_request(username, current_balance, grace):
    ...
    if not valid_withdrawal_request(current_balance, amount_to_withdraw, grace):
        print("The amount you requested {}, is too much. Sorry!")
        return False
...
assert valid_withdrawal_request(current_balance=1000, requested_withdrawal=1000.1, grace=0)
    #With grace as a decimal or a fraction 
assert valid_withdrawal_request(current_balance=1000, requested_withdrawal=1000.1, grace=0.1) 
    #With grace same as the the amount of withdrawal 
assert valid_withdrawal_request(current_balance=1000, requested_withdrawal=1000, grace=1000)
    #With Invalid Pin
assert authenticated_pin(user_pin =5678, entered_pin=1234)
    #With withdrawal amount greater than [grace and current balance]
assert valid_withdrawal_request(current_balance=5000, requested_withdrawal=5501, grace=0.1) 
    #With current user balance zero
assert valid_withdrawal_request(current_balance=0, requested_withdrawal=1000, grace=0.1)  
    #With pin entered a stings
assert authenticated_pin(user_pin =5678, entered_pin="abcd"),"You have entered a String"
    #Checking the balance
assert valid_withdrawal_request(current_balance=1000, requested_withdrawal=0, grace=0.1)
    #With grace being negative fraction
assert valid_withdrawal_request(current_balance=1000, requested_withdrawal=1000, grace=-0.2)
    #With grace being negative fraction and amount being <= transaction limit
assert valid_withdrawal_request(current_balance=1000, requested_withdrawal=900, grace=-0.1)
    #With entering the corrent PIN
assert authenticated_pin(user_pin =5678, entered_pin=5678)
    #Request half the amount of the current balance
assert valid_withdrawal_request(current_balance=1000, requested_withdrawal = 500, grace=0.1)
